# -------------------------------
# Robust Kenya-shaped word cloud:
# - increased font sizes
# - no overlap (rectangle collision check)
# - all words fully inside Kenya polygon
# -------------------------------
...
#    width ≈ size_pt * char_width * nchar(word)
#    height ≈ size_pt * line_height
# -------------------------------
...
# 0. Install if needed (run once)
# install.packages(c("wordcloud","RColorBrewer","magick","png","sf","rnaturalearth"))
...
dev.off()
# now 'kenya_mask_inverted.png' is white Kenya on transparent background
...
par(mar = c(0, 0, 0, 0))
set.seed(123)
# scale: c(max_scale, min_scale) — adjust to increase font sizes
...
wordcloud(
  words = names(freqs),
  freq = freqs,
  scale = c(9, 1.5),         # ← larger words
  min.freq = 1,
  random.order = FALSE,
  rot.per = 0,               # ← all horizontal for readability
  use.r.layout = TRUE,       # ← tighter packing
